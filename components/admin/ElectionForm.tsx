...
import { CODE_TO_POSITION_TYPE } from "@/lib/elections-constants";
...
export default function ElectionForm({
  ...
  const [initialForm, setInitialForm] = useState<typeof electionForm | null>(
    null
  );
        ...
        if (res.success && res.election) {
          ...
          const formValues = {
            ...
            dateOuverture: e.dateOuverture
              ? new Date(e.dateOuverture).toISOString().slice(0, 16)
              : "",
            dateCloture: e.dateCloture
              ? new Date(e.dateCloture).toISOString().slice(0, 16)
              : "",
            dateClotureCandidature: e.dateClotureCandidature
              ? new Date(e.dateClotureCandidature).toISOString().slice(0, 16)
              : "",
            dateScrutin: e.dateScrutin
              ? new Date(e.dateScrutin).toISOString().slice(0, 16)
              : "",
            nombreMandats: e.nombreMandats || 1,
            quorumRequis: e.quorumRequis || 0,
            majoriteRequis: e.majoriteRequis || "Absolue",
          };
          setElectionForm(formValues);
          setInitialForm(formValues);
          setPositions(e.positions || []);
        } else {
          toast.error(res.error || "Impossible de charger l'élection");
        }
      } finally {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("modal-save", onModalSave as EventListener);
  }, []);
  ...
  const resolveTypeFromTemplate = (template: PosteTemplate): PositionType => {
    const upper = template.code.toUpperCase();
    return (
      CODE_TO_POSITION_TYPE[upper] ||
      CODE_TO_POSITION_TYPE[template.code] ||
    ...
    try {
      ...
      if (isEdit) {
        if (
          ...
          initialForm.dateClotureCandidature ===
            electionForm.dateClotureCandidature &&
        ...
        const dateClotureCandidature = new Date(
          electionForm.dateClotureCandidature
        );
        ...
        if (dateOuverture >= dateClotureCandidature) {
          toast.error(
            "La date d'ouverture doit être antérieure à la date de clôture des candidatures"
          );
          return;
        }
        ...
        if (dateClotureCandidature >= dateScrutin) {
          toast.error(
            "La date de clôture des candidatures doit être antérieure à la date du scrutin"
          );
          return;
        }
        ...
        if (dateCloture <= dateScrutin) {
          toast.error(
            "La date de clôture doit être postérieure à la date du scrutin"
          );
          return;
        }
        ...
        if (!res.success) {
          toast.error(
            res.error || "Erreur lors de la mise à jour de l'élection"
          );
          return;
        }
        toast.success("Élection mise à jour avec succès !");
        setDirty(false);
        setInitialForm({ ...electionForm });
      } else {
        ...
        const res = await createElection(
          {
            ...
            dateOuverture: new Date(electionForm.dateOuverture),
            dateCloture: new Date(electionForm.dateCloture),
            dateClotureCandidature: new Date(
              electionForm.dateClotureCandidature
            ),
            dateScrutin: new Date(electionForm.dateScrutin),
            nombreMandats: electionForm.nombreMandats,
  ...
  const reloadElection = async () => {
    ...
    const res = await getElectionById(electionId!);
    if (res.success && res.election) {
      setPositions(res.election.positions || []);
    }
  };
  ...
  const handleAddSelectedTemplates = async () => {
    ...
    const existingTemplateIds = new Set(
      positions.map((p: any) => p.posteTemplateId).filter(Boolean)
    );
    const selectedTemplates = postesTemplates.filter((t) =>
      selectedPostes.includes(t.id)
    );
    const newTemplates = selectedTemplates.filter(
      (t) => !existingTemplateIds.has(t.id)
    );
    const skipped = selectedTemplates.filter((t) =>
      existingTemplateIds.has(t.id)
    );

    if (newTemplates.length === 0) {
      toast.warning(
        "Les postes sélectionnés existent déjà dans cette élection"
      );
      return;
    }
    ...
    try {
      ...
      const positionsData = newTemplates.map((t) => ({
        type: resolveTypeFromTemplate(t),
        titre: t.libelle,
        description: t.description || `Poste de ${t.libelle.toLowerCase()}`,
        nombreMandats: t.nombreMandatsDefaut || 1,
        dureeMandat: t.dureeMandatDefaut || 24,
        conditions: "Être membre actif de l'association",
        posteTemplateId: t.id,
      }));
      const res = await addPositionsToElection(
        electionId!,
        positionsData as any
      );
      if (!res.success) {
        toast.error(res.error || "Erreur lors de l'ajout des postes");
      ...
      if (skipped.length > 0) {
        toast.info(
          `${res.positions?.length || 0} ajouté(s), ${skipped.length} ignoré(s) (déjà présents).`
        );
      } else {
        toast.success(
          `${res.positions?.length || newTemplates.length} poste(s) ajouté(s)`
        );
      }
      await reloadElection();
    } finally {
  ...
  const handleCreateCustom = async () => {
    ...
    if (!newTitle) {
      toast.error("Veuillez saisir un titre de poste");
      return;
    }
    const hasDuplicateTitle = positions.some(
      (p: any) => (p.titre || "").trim().toLowerCase() === newTitle
    );
    if (hasDuplicateTitle) {
      toast.error("Un poste avec ce titre existe déjà pour cette élection");
      return;
    }
    try {
      setAddingPositions(true);
      const res = await createCustomPosition(electionId!, {
        type: PositionType.MembreComiteDirecteur,
        ...customPositionForm,
        titre: customPositionForm.titre.trim(),
      });
      if (!res.success) {
        toast.error(res.error || "Erreur lors de l'ajout du poste");
        return;
      }
      toast.success("Poste personnalisé ajouté");
  ...
  const handleUpdatePosition = async (pos: any, patch: any) => {
    if (!patch || Object.keys(patch).length === 0) {
      toast.info("Aucune modification à enregistrer pour ce poste");
      return;
    }
  ...
  const handleDeletePosition = async (pos: any) => {
    ...
    if (!res.success) {
      toast.error(res.error || "Erreur lors de la suppression du poste");
      return;
    }
  ...
  const availableTemplates = postesTemplates.filter(
    (t) => !positions.some((p) => p.posteTemplateId === t.id)
  );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {isEdit ? "Modifier l'élection" : "Créer une nouvelle élection"}
        </CardTitle>
        <CardDescription>
          {isEdit
            ? "Mettez à jour les informations et les postes à pourvoir"
            : "Configurez l'élection et sélectionnez les postes"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  setElectionForm({ ...electionForm, titre: e.target.value });
                  markDirty();
                }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <Label
                htmlFor="dateClotureCandidature"
              ...
              <Input
                ...
                onChange={(e) => {
                  setElectionForm({
                    ...electionForm,
                    dateClotureCandidature: e.target.value,
                  });
                  markDirty();
                }}
                required
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  setElectionForm({
                    ...electionForm,
                    nombreMandats: parseInt(e.target.value),
                  });
                  markDirty();
                }}
              />
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  setElectionForm({
                    ...electionForm,
                    quorumRequis: parseInt(e.target.value),
                  });
                  markDirty();
                }}
              />
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  setElectionForm({ ...electionForm, majoriteRequis: value });
                  markDirty();
                }}
              >
                <SelectTrigger>
          ...
          {!isEdit && (
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 mb-4">
                Sélectionnez les postes qui seront ouverts aux candidatures et
                aux votes
              </p>
              <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-blue-800 dark:text-blue-200">
                    Postes sélectionnés: {selectedPostes.length}/
                    {postesTemplates.length}
                  </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {postesTemplates.length > 0 ? (
                  postesTemplates
                    .sort(
                      (a, b) =>
                        a.ordre - b.ordre || a.libelle.localeCompare(b.libelle)
                    )
                    .map((poste) => (
                      <div
                        key={poste.id}
                  ...
                  <div className="p-4 text-center text-gray-500 md:col-span-2 space-y-2">
                    <p>Aucun poste disponible.</p>
                    <Link
                      href="/admin/postes"
                      className="text-blue-600 underline text-sm"
                    >
                      Créer des postes dans Gestion des postes
                    </Link>
                  </div>
                )}
              </div>

              {selectedPostes.length === 0 && postesTemplates.length > 0 && (
                <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                  <div className="flex items-center">
                    <AlertCircle className="h-4 w-4 text-amber-600 mr-2" />
                    <span className="text-sm text-amber-800 dark:text-amber-200">
                      Veuillez sélectionner au moins un poste pour créer
                      l&apos;élection
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-6 border-t pt-6">
              <div>
                <div className="text-base font-semibold mb-2">
                  Postes à pourvoir ({positions.length})
                </div>
                <div className="space-y-3">
                  {positions.length > 0 ? (
                    positions.map((p) => (
                      <div
                        key={p.id}
                        className="p-3 border rounded-lg bg-slate-50/80 dark:bg-slate-900/40"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              type="number"
                              min={1}
                              defaultValue={p.nombreMandats || 1}
                              onBlur={(e) =>
                                Number(e.target.value) !==
                                  (p.nombreMandats || 1) &&
                                handleUpdatePosition(p, {
                                  nombreMandats: Number(e.target.value),
                                })
                              }
                            />
                          </div>
                    ...
                    <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-3">
                      Aucun poste pour cette élection. Ajoutez-en ci-dessous.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                <div className="text-base font-semibold mb-2">
                  Ajouter des postes depuis les templates
                </div>
                {availableTemplates.length === 0 ? (
                  <div className="text-sm text-gray-600 space-y-2">
                    <p>
                      Tous les templates disponibles sont déjà associés, ou
                  ...
                  <>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  disabled={exists}
                                  checked={selectedPostes.includes(t.id)}
                                  onChange={(e) => {
                                    if (exists) return;
                                    setSelectedPostes((prev) =>
                                      e.target.checked
                    ...
                    <div className="flex justify-end mt-3">
                      <Button
                        type="button"
                        onClick={() => void handleAddSelectedTemplates()}
                        disabled={
                          selectedPostes.length === 0 || addingPositions
                        }
                      >
                        {addingPositions
                          ? "Ajout…"
                          : "Ajouter les postes sélectionnés"}
                      </Button>
                    </div>
                  </>
                )}
              ...
              <div>
                ...
                {!showCustomPositionForm ? (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setShowCustomPositionForm(true)}
                  >
                    Ajouter un poste personnalisé
                  </Button>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setCustomPositionForm({
                            ...customPositionForm,
                            dureeMandat: Number(e.target.value),
                          })
                        }
                      />
                    </div>
                    ...
                    <div className="md:col-span-2 flex justify-end gap-2">
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => setShowCustomPositionForm(false)}
                      >
                        Annuler
                      </Button>
                      <Button
