...
import { updateProjet } from "@/actions/projets";
...
export function EditProjetDialog({ projet, open, onOpenChange, onSuccess }: EditProjetDialogProps) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    titre: projet.titre,
    description: projet.description,
    statut: projet.statut,
    dateDebut: projet.dateDebut ? format(new Date(projet.dateDebut), "yyyy-MM-dd") : "",
    dateFin: projet.dateFin ? format(new Date(projet.dateFin), "yyyy-MM-dd") : "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateProjet(form);
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Erreur lors de la mise à jour du projet");
      }
    } catch (error) {
