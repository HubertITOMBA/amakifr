...
import { Loader2, FolderKanban, ArrowLeft, Plus, Edit, Trash2, Users, MessageSquare, Calendar, FileText, UserPlus } from "lucide-react";
import { toast } from "react-toastify";
import { getProjetById, deleteSousProjet, retirerAffectation } from "@/actions/projets";
import { format } from "date-fns";
import { fr } from "date-fns/locale";
import { CreateSousProjetDialog } from "@/components/admin/projets/CreateSousProjetDialog";
import { EditSousProjetDialog } from "@/components/admin/projets/EditSousProjetDialog";
import { AffecterSousProjetDialog } from "@/components/admin/projets/AffecterSousProjetDialog";
...
export default function ProjetDetailPage() {
  ...
  const [editProjetOpen, setEditProjetOpen] = useState(false);
  const [affecterDialogOpen, setAffecterDialogOpen] = useState<any>(null);

  // Logger la consultation de la page
  useActivityLogger(`Détails du projet ${projetId}`, "Projet");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card className="mx-auto max-w-7xl shadow-lg border-blue-200 dark:border-gray-700 !pt-0">
          <CardHeader className="bg-gradient-to-r from-blue-500/90 via-blue-400/80 to-blue-500/90 dark:from-blue-700/50 dark:via-blue-600/40 dark:to-blue-700/50 text-white !pt-0 rounded-t-lg">
            <div className="flex items-center justify-between pt-4 sm:pt-6 pb-4 sm:pb-6">
              ...
              <Button
                onClick={() => setEditProjetOpen(true)}
                className="bg-white/10 hover:bg-white/20 border-white/30 text-white hover:text-white"
              >
        ...
        {editProjetOpen && (
          <EditProjetDialog
            projet={projet}
            open={editProjetOpen}
            onOpenChange={setEditProjetOpen}
            onSuccess={loadProjet}
          />
        )}
