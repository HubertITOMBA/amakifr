...
import { updateTypeCotisationMensuelle } from "@/actions/cotisations-mensuelles";
import type { CategorieTypeCotisation } from "./CreateTypeCotisationDialog";
...
export interface TypeCotisationForEdit {
  id: string;
  nom: string;
  description?: string;
  montant: number;
...
interface EditTypeCotisationDialogProps {
  type: TypeCotisationForEdit | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export function EditTypeCotisationDialog({
  type,
  open,
  onOpenChange,
  onSuccess,
}: EditTypeCotisationDialogProps) {
  ...
  useEffect(() => {
    if (type && open) {
      setFormData({
        ...
        montant: type.montant ?? 0,
        obligatoire: type.obligatoire ?? true,
        actif: type.actif ?? true,
        ordre: type.ordre ?? 0,
        categorie: (type.categorie as CategorieTypeCotisation) ?? "Divers",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await updateTypeCotisationMensuelle({
        id: type.id,
      ...
      if (result.success) {
        toast.success("Type de cotisation mis à jour");
        onOpenChange(false);
        onSuccess?.();
      } else {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-2xl font-bold text-white mb-1">
                  Modifier le type de cotisation
                </DialogTitle>
                <DialogDescription className="text-blue-100 text-sm">
                  Modifier les paramètres du type
                </DialogDescription>
              </div>
            </div>
            <Button
                ...
                <Select
                  ...
                  onValueChange={(v: CategorieTypeCotisation) =>
                    setFormData({
                      ...formData,
                      categorie: v,
                      aBeneficiaire: v === "Assistance" ? true : formData.aBeneficiaire,
                    })
                  }
                >
              ...
              <div className="space-y-1">
                ...
                <div className="p-2 sm:p-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-md rounded-tl-none border border-slate-200 dark:border-slate-600 border-t-0 shadow-sm">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <Label
                      htmlFor="edit-type-aBeneficiaire"
                      className="cursor-pointer text-sm font-medium text-slate-900 dark:text-slate-100"
                    >
                      Type assistance
